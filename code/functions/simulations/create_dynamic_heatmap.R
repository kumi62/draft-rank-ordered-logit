...
create_dynamic_heatmap = function(simulation_results, player_order) {
  
  # Set gganimate specs
  options(gganimate.dev_args = list(width = 8, height = 6, units = 'in', res=320))
  
  prob_pick_range = range(simulation_results$pick)
  obs_pick_range = simulation_results %>%
    filter(prob == 1) %>%
    .$pick %>%
    range()
  ...
  draft_plot_df = expand.grid(
    pick_sim_start = obs_pick_range[1]:obs_pick_range[2],
    player = player_order,
    pick = prob_pick_range[1]:prob_pick_range[2]
  ) %>%
    left_join(simulation_results %>% select(-team), join_by(player, pick, pick_sim_start)) %>%
    left_join(team_order, join_by(pick)) %>%
    group_by(pick_sim_start, player) %>%
    filter(!(max(prob, na.rm = T) == 1 & is.na(prob))) %>%
    ungroup() %>%
    group_by(pick_sim_start, pick) %>%
    filter(!(max(prob, na.rm = T) == 1 & is.na(prob))) %>%
    ungroup() %>%
    ...
    filter(pick <= prob_pick_range[2]) %>%
    mutate(team = factor(team, levels = team_order$team[prob_pick_range[1]:prob_pick_range[2]]))
  
  # Create the static heatmap
  dynamic_heatmap = draft_plot_df %>%
    filter(player %in% head(player_order, prob_pick_range[2])) %>%
    ggplot() +
    ...
    transition_time(pick_sim_start)
