...
functions{
  ...
  // Model likelihood
  real rol_lpmf_xyz(int[] x, int[] x_agencies, vector theta, real[] x_heights, vector beta_heights, real[] x_propsr, vector beta_propsr, int[] x_overage, vector beta_overage, int r, int R, int is_draft) {
    ...
    if (is_draft == 0) {
      for (j in 1:R) {
        eta_vec[j] = theta[x[j]] + beta_heights[x_agencies[1]] * x_heights[j] + beta_propsr[x_agencies[1]] * x_propsr[j] + beta_overage[x_agencies[1]] * x_overage[j];
      }
    }
    ...
    for (i in 1:r) {
      ...
      if (is_draft == 1) {
        for (j in i:R) {
          eta_vec[j] = theta[x[j]] + beta_heights[x_agencies[i]] * x_heights[j] + beta_propsr[x_agencies[i]] * x_propsr[j] + beta_overage[x_agencies[i]] * x_overage[j];
        }
      }
...
parameters {
  ...
  vector[A-1] beta_heights_raw;
  vector[A-1] beta_propsr_raw;
  vector[A-1] beta_overage_raw;
...
transformed parameters {
  ...
  vector[A] beta_heights = append_row(beta_heights_raw, -sum(beta_heights_raw));
  vector[A] beta_propsr = append_row(beta_propsr_raw, -sum(beta_propsr_raw));
  vector[A] beta_overage = append_row(beta_overage_raw, -sum(beta_overage_raw));
...
model {
  ...
  for (i in 1:N) {
    out[i] = rol_lpmf_xyz(x[i,:], x_agencies[i,:], theta, x_heights[i,:], beta_heights, x_propsr[i,:], beta_propsr, x_overage[i,:], beta_overage, r[i], R[i], is_draft[i]);
  }
  ...
  beta_heights ~ normal(0.0, 1.0);
  beta_propsr ~ normal(0.0, 1.0);
  beta_overage ~ normal(0.0, 1.0);
